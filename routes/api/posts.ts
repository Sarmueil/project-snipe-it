...
router.get("/:id", async (req: any, res: Response, next: NextFunction) => {
  ...
  var postData:any = await getPosts({ _id: postId });
  postData = postData[0];
...
router.put("/:id/like", async (req: any, res: Response, next: NextFunction) => {
  var postId = req.params.id;
...
async function getPosts(filter: any) {
